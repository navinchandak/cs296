...
namespace cs296
{
  ...
  //! Simulation settings. Some can be controlled in the GUI.
		/** This struct defines a number of settings which has to be used in the GUI(OpenGL settings) and Box2d configuration 
		and it has the default values stored for these variables too. So in the constructor or base_sim_t, we use a instance
		of this struct to retrieve the value of various settings used to initialize the world of Box2d. Similarly this struct's instance
		has also been used in create_gui() function in main.cpp to supply the various parameters for GUI creation */
  struct settings_t
  {
  ...
  /** this struct just stores the name of the simulation, and also stores an instance of the dominos_t class(which is derived from the base_sim_t class). */
  struct sim_t
  {
    const char *name;
  ...
  const int32 k_max_contact_points = 2048;  
		/** This struct stores the details of a contact between two fixtures(i.e.overlap between two fixtures) . In the context of this project, it is used in pre_solve to store the contact fixtures, positions, normals, and state*/
 ...
 /** This class controls the entire simulation of the world. It creates the world object, and the dominos_t class which inherits from this adds all the objects (the pulleys, dominos etc) to the world created by it. It also links the debug_draw_t class defined in render.cpp with the Box2d so that graphics work properly.
	It also defines and implements the step function for the simulation, where it calls the step function of Box2d . It enables the features such as pause,statistics, and profile viewing and helps to take various settings from the GUI (draw_joints, draw_AABBs,profile viewing etc) and pass it onto Box2d settings,	so that the user can get output as required
It also defines the pre_solver class, which stores all the information about the contacts before the solving time, and then can display the contact points on the screen if needed in the Step() function
It also defines many other functions which are not relevant in this example project, such as shift_mouse_down, mouse_up etc
	*/ 
